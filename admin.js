...
document.getElementById("migrate-btn").addEventListener("click", async function() {
    ...
    try {
        ...
        if (titleLower.includes('консерва') || titleLower.includes('паштет') || titleLower.includes('тушонка') || titleLower.includes('кілька')) categories.push('autoclave');
    ...
    loadProducts();
        ...
        logEl.innerHTML += `<br><b style="color:red">КРИТИЧНА ПОМИЛКА: ${e.message}</b><br>Будь ласка, зробіть скріншот цього тексту.`;
